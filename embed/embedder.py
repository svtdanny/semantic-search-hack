...
class IndexEmbeddingModel:
    def __init__(self):
        ...
        self.model, self.vis_processor = video_clip.load_model(
            eval_config, settings.device
        )
    ...
    def get_video_embedding(self, link):
        ...
        embedding = video2embedding(
            io.BytesIO(rsp.read()), self.model, self.vis_processor, settings.device
        )
        return embedding

    def get_query_embedding(self, query):
        embedding = (
            video_clip.embed_text_itc(self.model, query, settings.device).cpu().numpy()
        )
        return embedding
